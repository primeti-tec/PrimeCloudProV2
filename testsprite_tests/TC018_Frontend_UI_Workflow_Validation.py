...
async def run_test():
    ...
    try:
        ...
        # -> Input the password and click the continue button to proceed with login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click the continue button to submit the password and login.
        ...
        frame = context.pages[-1]
        # Click the continue button to submit the new verification code and complete login.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Use an alternative method to complete login or request a new verification code.
        frame = context.pages[-1]
        # Click 'Utilize outro método' to try an alternative login verification method.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the button to send a code to the email to proceed with login.
        frame = context.pages[-1]
        # Click the button to send a verification code to the email.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the verification code received by email and click the continue button to complete login.
        frame = context.pages[-1]
        # Input the verification code received by email.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Verifique seu e-mail').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=para continuar em').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=sergio.louzan@dicorel.com').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Não recebeu um código? Reenviar (25)').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Você está entrando de um novo dispositivo. Estamos solicitando verificação para manter sua conta segura.').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Continuar').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Utilize outro método').first).to_be_visible(timeout=30000)
