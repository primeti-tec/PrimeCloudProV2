...
async def run_test():
    ...
    try:
        ...
        # -> Input the team owner's email and continue.
        frame = context.pages[-1]
        # Input the team owner's email address.
        ...
        # -> Input the password and click continue to log in.
        frame = context.pages[-1]
        # Input the team owner's password.
        ...
        frame = context.pages[-1]
        # Click the continue button to verify and complete login.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the correct verification code and click continue to complete login.
        frame = context.pages[-1]
        # Input the verification code received by email.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        ...
        # -> Use an alternative method to verify or wait for the correct verification code to proceed with login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Role updated to superuser').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The test plan execution failed because the role update and access revocation did not apply the correct permission updates immediately as expected.")
