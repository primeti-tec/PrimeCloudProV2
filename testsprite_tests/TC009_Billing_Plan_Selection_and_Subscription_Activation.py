...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Click the continue button to proceed after verification code input
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the verification code and click the continue button to complete login.
        frame = context.pages[-1]
        # Input the verification code received by email
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        ...
        # -> Use an alternative method to complete verification or wait for a new code to be sent and input the correct code.
        frame = context.pages[-1]
        ...
        # -> Click 'Utilize outro método' to proceed with alternative verification method selection.
        frame = context.pages[-1]
        # Click 'Utilize outro método' to try an alternative verification method
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Subscription Pending Activation').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The subscription did not transition to active state and usage tracking was not enabled as required by the test plan.")
