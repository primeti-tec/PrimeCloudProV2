...
async def run_test():
    ...
    try:
        ...
        # -> Input password 145@DicoreL and click continue to login
        frame = context.pages[-1]
        # Input the password for user sergio.louzan@dicorel.com
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Access Granted to Tenant B Resources').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Role-based authorization did not enforce tenant data isolation. Access to tenant B resources should be denied for user in tenant A.")
