...
async def run_test():
    ...
    try:
        ...
        # -> Input the email verification code and click continue to complete login.
        frame = context.pages[-1]
        # Input the email verification code.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('123456')
        

        frame = context.pages[-1]
        # Click the continue button to proceed after email verification.
        ...
        # -> Click the 'Utilize outro método' link to try an alternative verification method to complete login.
        frame = context.pages[-1]
        # Click 'Utilize outro método' to try an alternative verification method for login.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the 'Enviar código para sergio.louzan@dicorel.com' button to resend the verification code.
        frame = context.pages[-1]
        # Click the button to resend the verification code to the user's email.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the new verification code and click continue to complete login.
        frame = context.pages[-1]
        # Input the new verification code received by email.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('654321')
        

        frame = context.pages[-1]
        # Click the continue button to proceed after inputting the verification code.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Retry inputting the verification code correctly and click continue to complete login.
        frame = context.pages[-1]
        # Re-input the verification code to ensure it is properly entered.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/div/div/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('654321')
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Successful SFTP credential creation for tenant').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: SFTP credential creation or password reset process did not complete successfully or proper logging was not found as per the test plan.")
