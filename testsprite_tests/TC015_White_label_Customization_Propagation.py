...
async def run_test():
    ...
    try:
        ...
        # -> Use 'Utilize outro método' link to try alternative login method to bypass email verification for faster access to tenant UI
        frame = context.pages[-1]
        # Click 'Utilize outro método' link to try alternative login method
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the 'Enviar código para sergio.louzan@dicorel.com' button to send the verification code and proceed with login.
        frame = context.pages[-1]
        # Click 'Enviar código para sergio.louzan@dicorel.com' button to send verification code
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the verification code received by email and click continue to complete login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click the continue button to submit verification code
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the 'Utilize outro método' link to try an alternative login method.
        frame = context.pages[-1]
        # Click 'Utilize outro método' link to try alternative login method
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Request user to provide the valid verification code received by email to proceed with login.
        frame = context.pages[-1]
        # Click 'Utilize outro método' link to try alternative login method
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Tenant Branding Successfully Applied').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Tenant-specific white-label configurations including logos, colors, favicon, and SMTP email settings did not propagate correctly throughout the UI and email communications as per the test plan.")
