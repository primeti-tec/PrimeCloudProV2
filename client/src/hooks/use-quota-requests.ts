  ...
  return useMutation({
    mutationFn: async ({ accountId, requestedQuotaGB, reason }: { accountId: number; requestedQuotaGB: number; reason?: string }) => {
      ...
      const res = await apiRequest(url, {
        method: api.quotaRequests.create.method,
        body: JSON.stringify({ requestedQuotaGB, reason }),
      });
      return res;
    },
...
export function useApproveQuotaRequest() {
  ...
  return useMutation({
    mutationFn: async ({ id, note }: { id: number; note?: string }) => {
      ...
      const res = await apiRequest(url, {
        method: api.quotaRequests.approve.method,
...
export function useRejectQuotaRequest() {
  ...
  return useMutation({
    mutationFn: async ({ id, note }: { id: number; note?: string }) => {
      ...
      const res = await apiRequest(url, {
        method: api.quotaRequests.reject.method,
        body: JSON.stringify({ note }),
      });
      return res;
    },
